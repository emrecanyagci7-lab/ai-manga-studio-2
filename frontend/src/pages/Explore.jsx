    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      <div className="mb-10">
        <div className="text-xs tracking-[0.3em] uppercase text-violet-400 mb-2 flex items-center gap-2">
          <Compass className="w-3 h-3" /> // Topluluk Akışı
        </div>
        <h1 className="font-display text-5xl md:text-6xl tracking-widest uppercase">Keşfet</h1>
        <p className="mt-2 text-slate-400">Toplulukça yayınlanan mangalar. Oku, ilham al.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="ink-card p-14 text-center">
          ...
          <div className="font-display text-3xl tracking-widest uppercase mb-3">Henüz Yayınlanan Yok</div>
          <p className="text-slate-400">Manganı Keşfet akışında yayınlayan ilk kişi ol.</p>
        </div>
      ) : (
              ...
              <Link to={`/manga/${m.id}`} data-testid={`explore-card-${m.id}`}>
                <div className="ink-card overflow-hidden hover:border-violet-500/40 hover:-translate-y-1 transition-transform group">
                  ...
                  <div className="p-3">
                    ...
                    <div className="text-[10px] tracking-widest uppercase text-slate-500 mt-1">{GENRE_TR[m.genre] || m.genre}</div>
                  </div>
                </div>
              </Link>
            </motion.div>
