  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      <div className="flex items-end justify-between mb-10">
        <div>
          <div className="text-xs tracking-[0.3em] uppercase text-violet-400 mb-2">// My Vault</div>
          <h1 className="font-display text-5xl md:text-6xl tracking-widest uppercase">Your Manga</h1>
        </div>
        <Link to="/create">
      ...
      {loading ? (
        ...
        <div className="ink-card p-14 text-center">
          ...
          <div className="font-display text-3xl tracking-widest uppercase mb-3">Empty Studio</div>
          <p className="text-slate-400 mb-6">Sketch your first idea to bring a manga to life.</p>
          <Link to="/create"><Button className="bg-violet-600 hover:bg-violet-500">Start Creating</Button></Link>
        </div>
      ) : (
              ...
              <Link to={`/manga/${m.id}`} data-testid={`library-card-${m.id}`}>
                <div className="ink-card overflow-hidden hover:border-violet-500/40 hover:-translate-y-1 transition-transform group">
                  <div className="aspect-[2/3] bg-slate-900 halftone flex items-center justify-center relative">
                    ...
                    {m.is_published && (
                      <span className="absolute top-2 right-2 text-[9px] tracking-widest uppercase px-2 py-1 rounded bg-emerald-500/25 text-emerald-200">Public</span>
                    )}
                  </div>
                  <div className="p-3">
                    <div className="font-display text-lg tracking-wide truncate group-hover:text-violet-300">{m.title}</div>
                    <div className="text-[10px] tracking-widest uppercase text-slate-500 mt-1">{m.genre} · {m.chapter_count} ch</div>
                  </div>
                </div>
              </Link>
            </motion.div>
